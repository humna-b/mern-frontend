...
export default function BookingModal({ isOpen, onClose, coach }: BookingModalProps) {
  ...
  // Safely build an ISO string from a date + "HH:MM" time.
  // Returns null if the date or time is missing/invalid (instead of crashing).
  const buildISO = (baseDate: Date, time: string): string | null => {
    if (!baseDate || !time || !time.includes(":")) return null
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const startTimeISO = buildISO(date, startTime)
        const endTimeISO = buildISO(date, endTime)
  ...
  const handleCreateSession = async () => {
    ...
    const sessionStartTime = buildISO(date, startTime)
    const sessionEndTime = buildISO(date, endTime)

    if (!sessionStartTime || !sessionEndTime) {
      setTimeError("Please pick a valid date and start/end time.")
      setIsLoading(false)
      return
    }
